...
namespace FindTheRoot
{
    class FindTheRoot
    {
        ...
        public static double binarySearch(int p,int q,int r,int s,int t,int u,double start,double end)
        {
            if (start < end)
            {
                double mid = (start + end) / 2;
                double result = f(p,q,r,s,t,u,mid);
                if (result > -eps && result < eps)
                {
                    return mid;
                }
                //double mid2 =mid + 0.01;
                //double result2 = (p * Math.Exp(-mid2)) + (q * Math.Sin(mid2)) + (r * Math.Cos(mid2)) + (s * Math.Tan(mid2)) + (t * mid2 * mid2) + u;
